...
const options = {
  ...
  camelCase: true, // Models文件中代码是否使用驼峰发命名
  fileNameCamelCase: true, // Model文件名是否使用驼峰法命名，默认文件名会使用表名，如 `user_post.js`；如果为 true，则文件名为 `userPost.js`
...
const automate = new Automate(dbOptions.dev, options);
(async function main() {
